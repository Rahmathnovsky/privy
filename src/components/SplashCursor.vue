...
onMounted(() => {
  ...
  // Privy Red-themed color generation
  function generateColor() {
    // Generate colors in red/pink/orange spectrum for Privy theme
    let hue = Math.random() * 0.1; // Red hues (0-0.1)
    if (Math.random() > 0.5) hue = 0.95 + Math.random() * 0.05; // Also include pink/magenta
    let c = HSVtoRGB(hue, 0.8 + Math.random() * 0.2, 1.0);
    // Bright color intensity for visibility
    c.r *= 0.8;
    c.g *= 0.15;
    c.b *= 0.15;
    return c;
...
<template>
  <div class="splash-cursor-container">
    <canvas ref="canvasRef" id="fluid" class="splash-canvas"></canvas>
  </div>
</template>
